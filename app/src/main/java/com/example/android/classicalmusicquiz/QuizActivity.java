...
// TOD (1): Have this Activity implement ExoPlayer.EventListener and add the required methods.
public class QuizActivity extends AppCompatActivity implements View.OnClickListener, ExoPlayer.EventListener {
    ...
    private void initializeMediaSession() {
        // TOD (1): Create a method to initialize the MediaSession. It should create the MediaSessionCompat object,
        ...
        // Do not let the Media Buttons restart the player when the app is not visible
        ...
        mMediaSession.setCallback(new innerClass());

        // Start the Media Session since the activity is active.
        mMediaSession.setActive(true);

        // TOD (2): Call the method to show the notification, passing in the PlayBackStateCompat object.
        showNotification(mStateBuilder.build());
    }

// TOD (1): Create a method that shows a MediaStyle notification with two actions (play/pause, skip to previous). Clicking on the notification should launch this activity. It should take one argument that defines the state of MediaSession
    /**
    ...
    private void initializePlayer(Uri mediaUri) {
        if (mExoPlayer == null) {
            ...
            // TOD (2): Set the ExoPlayer.EventListener to this activity
    ...
    private void releasePlayer() {
        mNotificationManager.cancelAll(); // This is done so that the media session controller for external clients
        // will not be hanging around even when the activity of the app has been destroyed.
        mExoPlayer.stop();
        mExoPlayer.release();
        mExoPlayer = null;
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        releasePlayer();
        // when the activity is destroyed, set the media session to inactive.
    ...
    public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
        if((playbackState == ExoPlayer.STATE_READY)&& playWhenReady){
            // We are playing
            Log.d("TAG","playbackState: PLAYING");
            mStateBuilder.setState(PlaybackStateCompat.STATE_PLAYING,
                    mExoPlayer.getCurrentPosition(), 1f);

        } else if ((playbackState == ExoPlayer.STATE_READY)){
            // We are paused
            Log.d("TAG2","playbackState: PAUSED");
    ...
    // TOD (3): Add conditional logging statements to the onPlayerStateChanged() method that log when ExoPlayer is playing or paused.


    // TOD (2): Create an inner class that extends MediaSessionCompat.Callbacks, and override the onPlay(), onPause(),
//  and onSkipToPrevious() callbacks. Pass an instance of this class into the MediaSession.setCallback() method in the method you created in TODO 1.

    /**
     * Media Session Callbacks, where all external clients control the player.
     */
    private class innerClass extends MediaSessionCompat.Callback{
        @Override
        public void onPlay() {
            mExoPlayer.setPlayWhenReady(true);
        }
    ...
    // TOD (1): Create a static inner class that extends Broadcast Receiver and implement the onReceive() method.
    //TOD (2): Call MediaButtonReceiver.handleIntent and pass in the incoming intent as well as the MediaSession object to forward the intent to the MediaSession.Callbacks
    /**
